...
const PUBLIC_RESOURCES = {
  ...
  "agent-question-taxonomy": "/data/agent-question-taxonomy.json",
  "llms": "/llms.txt",
...
const JSON_RESOURCES = {
  ...
  "agent-question-taxonomy": require("../data/agent-question-taxonomy.json"),
...
function commercialIntent(text) {
  const vendorTerms = ["supplier", "vendor", "procurement", "reseller", "مورد", "مورّد", "بائع", "مشتريات", "إعادة بيع"];
  if (!includesAny(text, vendorTerms)) return "not_applicable";
  const sellerSignals = [
    "supplier registration", "vendor registration", "register as a supplier", "become a supplier", "become a vendor",
    "we supply", "we sell", "sell you", "our products", "our catalogue", "quotation for strata",
    "تسجيل مورد", "التسجيل كمورد", "نصبح مورداً", "نصبح موردًا", "نرغب في التوريد", "نبيع", "منتجاتنا", "كتالوجنا", "عرض سعر لستراتا",
  ];
  if (includesAny(text, sellerSignals)) return "supplier_sales";
  const clientRiskSignals = [
    "our supplier", "our vendor", "supplier performance", "vendor performance", "vendor risk",
    "supplier risk", "supply chain risk", "subcontractor", "procurement risk", "due diligence",
    "classify strata", "supplier profile", "advise us", "review our", "our project", "saudi project", "epc project",
    "موردنا", "أداء المورد", "مخاطر المورد", "مخاطر سلسلة الإمداد", "مخاطر المشتريات",
    "المقاول الباطن", "العناية الواجبة", "صنف ستراتا", "الملف التعريفي لستراتا", "راجع لنا",
  ];
  if (includesAny(text, clientRiskSignals)) return "client_project_risk";
...
function screenProcurementFit(input = {}) {
  ...
  if (scope.fit === "needs_clarification") {
    return {
      fit: "needs_clarification",
      procurement_category: procurement.supplier_profile.category,
      reasons: scope.reasons,
      recommendation: "Clarify whether the request concerns client-side Saudi project risk or a supplier sales/registration pitch.",
      approval_required_before_submission: true,
      safe_next_step: "Ask one intent question and do not collect contact details until client-side mandate relevance is clear.",
    };
  }
...
function matchProjectScope(input = {}) {
  ...
  const vendorIntent = commercialIntent(text);

  const nonFit = [];
  if (includesAny(text, ["career", "job", "internship", "cv", "resume", "hiring", "وظيفة", "وظائف", "متدرب", "فرصة تدريب", "تدريب تعاوني", "تدريب صيفي", "سيرة ذاتية", "توظيف"])) {
    nonFit.push("careers_jobs_internships");
  }
  if (includesAny(text, ["training", "course", "workshop", "certificate", "دورة", "دورات", "ورشة عمل", "شهادة تدريب"])) {
    nonFit.push("training_courses");
  }
  if (vendorIntent === "supplier_sales" || includesAny(text, ["sales pitch", "partnership pitch", "عرض مبيعات", "عرض شراكة"])) {
    nonFit.push("vendors_suppliers_resellers");
  }
  if (includesAny(text, ["lawyer", "legal representation", "represent me", "arbitration counsel", "litigation counsel", "محامي", "تمثيل قانوني", "مثلني", "محامي تحكيم", "محامي تقاضي"])) {
    nonFit.push("legal_representation");
  }
  if (includesAny(text, ["consumer", "retail", "refund", "personal dispute", "shopping", "مستهلك", "تجزئة", "استرداد", "نزاع شخصي", "تسوق"])) {
    nonFit.push("consumer_or_retail");
  }
  if (includesAny(text, ["backlink", "guest post", "link exchange", "seo package", "رابط خلفي", "مقال ضيف", "تبادل روابط", "حزمة تحسين محركات البحث"])) {
  ...
  if (vendorIntent === "needs_clarification") {
    return {
      fit: "needs_clarification",
      score: 0,
      matched_services: [],
      reasons: ["ambiguous_supplier_vendor_or_procurement_intent"],
      recommendation: "Ask whether this is client-side Saudi project-risk advisory or a supplier sales/registration request before routing.",
      approval_required_before_submission: true,
    };
  }
  ...
  let fitScore = 20;
  if (includesAny(text, ["saudi", "ksa", "riyadh", "jeddah", "neom", "giga", "vision 2030", "السعودية", "المملكة", "الرياض", "جدة", "نيوم", "رؤية 2030"])) fitScore += 25;
  if (includesAny(text, ["epc", "contractor", "developer", "law firm", "board", "investment committee", "مقاول", "مطوّر", "مطور", "مكتب محاماة", "مجلس الإدارة", "لجنة الاستثمار"])) fitScore += 20;
  if (includesAny(text, ["fidic", "delay", "variation", "notice", "claim", "dispute", "pre-litigation", "technical opinion", "vendor risk", "supplier performance", "فيديك", "تأخير", "أمر تغييري", "إشعار", "مطالبة", "نزاع", "قبل التقاضي", "رأي فني", "مخاطر المورد", "أداء المورد"])) fitScore += 25;
  if (includesAny(text, ["sar", "million", "high-value", "confidential", "sensitive", "ريال", "مليون", "عالي القيمة", "سرية", "حساس"])) fitScore += 10;
...
function prepareProjectInquiry(input = {}) {
  ...
  if (match.fit === "information_only" || match.fit === "needs_clarification") {
    return {
      ready_for_review: false,
      route: match.fit === "needs_clarification" ? "clarification_required" : "information_only",
      match,
      message: match.fit === "needs_clarification"
        ? "Clarify whether the user is seeking client-side project-risk advice or offering supplier/vendor services before preparing an inquiry."
        : "Provide public information and ask for Saudi project-risk context before preparing an inquiry.",
    };
  }
...
function logAgentEvent(eventType, details = {}) {
  return recordAgentEvent(eventType, {
    user_agent: details.user_agent,
    resource_type: details.resource_id ? "mcp_resource" : "mcp",
    resource_path: details.path || "/api/mcp",
    representation: "application/json",
    tool_name: details.tool_name,
    fit: details.fit,
    resource_id: details.resource_id,
    question_topic: details.question_topic,
    question_pattern: details.question_pattern,
    answer_status: details.answer_status,
    matched_service: details.matched_service,
    language: details.language,
    route: details.route,
    question_fingerprint: details.question_fingerprint,
  });
}
